...
# Clean-up data
cum_data = cum_data[, -which(names(cum_data) %in% c("Country.Region"))]
diff_data = diff_data[, -which(names(diff_data) %in% c("Country.Region"))]
...
output = paste(output, "@difference",
               paste(as.vector(unname(diff_data["US",2:length(diff_data)])), collapse = ","),
               paste(as.vector(unname(diff_data["Canada",2:length(diff_data)])), collapse = ","),
               sep = "\n")
```
